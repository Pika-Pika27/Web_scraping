...
def process_onlinekhabar(soup):
    ...
    text_data=content.text.strip().encode('latin1').decode('utf-8')
    return text_data


def process_nayapatrika(soup):
    content = soup.find('div', class_='news-details-content')
    ny=content.text
    return ny


def process_himalkhabar(soup):
    content = soup.find('div',class_='')
    hk=content.text
    return hk


def process_gorkhapatra(soup):
    content = soup.find('div',class_=)